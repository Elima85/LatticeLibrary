...
#include "exception.h"
#include <math.h>
#include <tgmath.h> // for round and pow
#include "neighbor.h"
using namespace std;
...
// neighbor distances
//#define CCd001 1
//#define CCd011 1.414213562373095
//#define CCd111 1.732050807568877
//
//#define CCd012 2.236067977499790
//#define CCd112 2.449489742783178
//#define CCd122 3
//
//#define CCd013 3.162277660168380
//#define CCd023 3.605551275463989
//#define CCd113 3.316624790355400
//#define CCd123 3.741657386773941
//#define CCd133 4.358898943540674
//#define CCd223 4.123105625617661
//#define CCd233 4.690415759823430
...
public:
	...
	// Get coordinates
	// For CC, (x,y,z) = (c,r,l). Could use indexTo[R,C,L]().
	 ...
	 */
//	vector<Neighbor> get6Neighbors(int r, int c, int l) const {
...
//		return neighbors;
	 ...
	 */
//	vector<Neighbor> get6Neighbors(int i) const {
...
//		return get18Neighbors(this->indexToR(i), this->indexToC(i), this->indexToL(i));
	 ...
	 */
//	vector<Neighbor> get26Neighbors(int r, int c, int l) const {
...
//		neighbors = get18Neighbors(r,c,l);
	 ...
	 */
//	vector<Neighbor> get26Neighbors(int i) const {
...
//		return get26Neighbors(this->indexToR(i), this->indexToC(i), this->indexToL(i));
	 ...
	 */
//	vector<Neighbor> get124Neighbors(int r, int c, int l) const {
...
//		return neighbors;
	 ...
	 * Returns the neighbors within a \f5\times5\times5\f cube with its center at \f$(r,c,l)\f$.
	 */
//	vector<Neighbor> get124Neighbors(int i) const {
...
//		return get124Neighbors(this->indexToR(i), this->indexToC(i), this->indexToL(i));
	 ...
	 */
//	vector<Neighbor> get342Neighbors(int r, int c, int l) const {
...
//		return neighbors;
	 ...
	 * Returns the neighbors within a \f5\times5\times5\f cube with its center at \f$(r,c,l)\f$.
	 */
//	vector<Neighbor> get342Neighbors(int i) const {
...
//	vector<Neighbor> getNeighbors(int i, int N) const {
	 ...
	 */
	void downsample(const CImage<T>* original, double newSpelVolume, T* data) {
		...
		//int oldNRows = original->getNRows();
		//int oldNColumns = original->getNColumns();
		//int oldNLayers = original->getNLayers();
		...
		this->nBands = nBands;
		data = new T[newNElements * nBands];
		...
		double squaredDistanceToCurrent, squaredDistanceToNeighbor;
		bool inside;
		for (int newE = 0; newE < newNElements; newE++) {
			...
			for (int oldE = 0; oldE < oldNElements; oldE++) { // Should be optimized to some search area!!
				...
				for (int i = 0; i < 3; i++) {
					squaredDistanceToCurrent += (newLocation[i] - oldLocation[i]) * (newLocation[i] - oldLocation[i]);
				}
				inside = true;
				squaredDistanceToNeighbor = 0;
				for (int n = 0; (n < nNeighbors) && inside; n++) {
					for (int i = 0; i < 3; i++) {
						squaredDistanceToNeighbor += (newNeighborLocations[n][i] - oldLocation[i]) * (newNeighborLocations[n][i] - oldLocation[i]);
					}
			...
			this->setElement(newE, (1/nSubSpels) * newIntensity);
		}
	}
